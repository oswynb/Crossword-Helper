...
tileLatex w Void = "|" <> (replicate (2 + w) ' ') <> "*"
tileLatex w (Tile x Nothing) = "|" <> (replicate (2 + w) ' ') <> [x]
tileLatex w (Tile x (Just n)) = "|" <> "[" <> (show n) <> "]" <> (replicate (w - intToWidth n) ' ') <> [x]

rowLatex :: Int -> [Tile] -> String
rowLatex clueCount ts =  "  " <> concat (map (tileLatex $ intToWidth clueCount) ts) <> "|.\n"

latexSolution :: Crossword -> String
latexSolution cw = concat
  [ latexHeader Solution
  , puzzleLatex cw
  , latexFooter
  ]

latexUnsolved :: Crossword -> String
latexUnsolved c = concat
  [ latexHeader Unsolved
  , puzzleLatex c
