...
instance Show Clue where
  ...
  show (Clue _pos num clue ans dir) = show num ++ " " ++ show dir ++ ": " ++ clue ++ " - " ++ ans
...
addEmptyClues c = c{acrossClues = acrossClues, downClues = downClues}
  where
    ...
    acrossPositions = filter (\(_, p) -> hasAcross p c) pairs
    downPositions   = filter (\(_, p) -> hasDown   p c) pairs
...
constructCrossword strings = addEmptyClues $ Crossword w h ts [] []
  where
    h = length strings
    w = length $ head strings
    ts = constructTileArray w h strings
...
numberTile x y = error $ "you wot numberTile " <> show x <> " " <> show y
